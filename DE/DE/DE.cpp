...
string strToBinary(string hexdec)
{
	long int i = 0;
	string bin = "";
...
string Encryption(string inputPlainText ,string key) {
...
int main() {
	...
	if (chose == "1") {
		...
		cout << "Enter The path of plain text File:              Note:Should be like that'C:/Users/omarw/Desktop/omar.txt'" << endl;
		cin >> filePath;
		cout << "Enter The path of Key File:              Note:Should be like that'C:/Users/omarw/Desktop/omar.txt'" << endl;
		cin >> keyPath;
		cout << "Enter Output destination:              Note:Should be like that'C:/Users/omarw/Desktop'" << endl;
		cin >> outputPath;
		string key;
		ifstream MyKey(keyPath);
		if (!MyKey) {
			cout << "Err Couldnt open the plain text file" << endl;
			return 0;

		}
		while (MyKey.get(keychar)) {                  // loop getting single characters
			key = key + keychar;
		}
		key = strToBinary(key);
		MyKey.close();
		...
		while (infile.get(letter)) {                  // loop getting single characters
			plainText = plainText + letter;
			if (count % 16 == 15) {
				cout << plainText;
				plainText = strToBinary(plainText);
				cout << plainText;
				//cout << Encryption(plainText, key) << endl;
				ofstream myfile;
				myfile.open(outputPath + "/outPutEncrypt.txt", std::ios_base::app);
				myfile << Encryption(plainText, key);
				myfile.close();
				plainText = "";
		...
		cout << "Enter The path of cypher text File:              Note:Should be like that'C:/Users/omarw/Desktop/omar.txt'" << endl;
		cin >> filePath;
		cout << "Enter The path of Key File:              Note:Should be like that'C:/Users/omarw/Desktop/omar.txt'" << endl;
		cin >> keyPath;
		cout << "Enter Output destination:              Note:Should be like that'C:/Users/omarw/Desktop'" << endl;
		...
		while (infile.get(letter)) {                  // loop getting single characters
			...
			if (count % 16 == 15) {
				cypherText = strToBinary(cypherText);
				
				//cout << Encryption(plainText, key) << endl;
				ofstream myfile;
				myfile.open(outputPath + "/outPutDencrypt.txt", std::ios_base::app);
				myfile << Decryption(cypherText, key);
				myfile.close();
				cypherText = "";
